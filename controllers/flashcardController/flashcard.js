...
const createFlashcard = async (req, res) => {
  try {
    ...
    if (user.fcmToken) {
      const message = {
        notification: {
          title: "🎉 Flashcard Baru Nih! 🎉",
          body: `Hai Champ! Kamu baru saja nambahin flashcard "${question}" di deck "${deckExists.name}". Ayo semangat belajar-nya! 💪📚 #ProgressBanget`,
        },
        token: user.fcmToken,
      };
...
const deleteFlashcard = async (req, res) => {
  try {
    ...
    // Hapus semua history yang terkait dengan flashcard ini
